...
TOKEN=$(curl -s -X POST "$ENDPOINT/auth/battery-login" \
  ...
  -d '{"battery_id": 1, "secret": "test123"}' \
  | grep -o '"access_token":"[^"]*"' \
  | cut -d'"' -f4)

if [ -z "$TOKEN" ]; then
  echo "Failed to get battery token - webhook will be anonymous"
  AUTH_HEADER=""
else
...
echo "  curl http://localhost:8000/batteries/1/errors"
echo ""
echo "Check notifications:"
echo "  curl http://localhost:8000/notifications"
