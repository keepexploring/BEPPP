...
class EnhancedBatteryAnalyticsDashboard(param.Parameterized):
    ...
    def view(self):
        ...
        # Battery dual-list selector - filter by capacity first
        def get_filtered_battery_options():
            """Get batteries filtered by capacity range"""
            filtered = [b for b in self.battery_options
                       if self.min_capacity <= b['capacity_wh'] <= self.max_capacity]
            return {f"Battery {b['battery_id']} ({b['brand']} {b['model']}, {b['capacity_wh']}Wh)": b['battery_id']
                   for b in filtered}

        battery_options_dict = get_filtered_battery_options()

        battery_selector = pn.widgets.CrossSelector(
            name='🔋 Select Batteries (move to right to include)',
            value=list(self.selected_batteries),
            options=battery_options_dict,
            height=300,
            sizing_mode='stretch_width'
        )
