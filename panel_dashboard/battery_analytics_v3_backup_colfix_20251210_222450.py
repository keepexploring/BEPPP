...
class EnhancedBatteryAnalyticsDashboard(param.Parameterized):
    ...
    def view(self):
        ...
        # STEP 3: Battery Selection (filtered by capacity)
        # Show ALL batteries that match capacity filter
        all_battery_options = {f"Battery {b['battery_id']} ({b['brand']} {b['model']}, {b['capacity_wh']}Wh)": b['battery_id']
                               for b in self.battery_options}

        battery_selector = pn.widgets.CrossSelector(
            name='3️⃣ Select Batteries (move to right to include in plots)',
            value=list(self.selected_batteries),
            options=all_battery_options,
            height=300,
            sizing_mode='stretch_width'
        )
